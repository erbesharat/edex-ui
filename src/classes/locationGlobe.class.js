class LocationGlobe {
          ...
          .get(
            {
              ...
              port: 443,
              path: "/" + ip,
              localAddress: window.mods.netstat.internalIPv4,
              agent: false
            },
            res => {
              let rawData = "";
              ...
              res.on("end", () => {
                this.parseResponse(rawData, ip).catch(e => {
                  window.mods.netstat.failedAttempts[e] =
                    (window.mods.netstat.failedAttempts[e] || 0) + 1;
                  if (window.mods.netstat.failedAttempts[e] > 2) return false;
                  let electron = require("electron");
                  electron.ipcRenderer.send(
                    "log",
                    "note",
                    "LocationGlobe: Error parsing data from ipinfo.now.sh"
                  );
  ...
  async updateConOnlineConnection() {
    let newgeo = {
      latitude: 35.69974841956453,
      longitude: 51.31925058373172
    };
